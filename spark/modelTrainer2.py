...
cls = DecisionTreeClassifier(featuresCol='features', labelCol='readmitted', maxDepth=30)
cls_model = cls.fit(train)
cal_run_time(start_time, time.time())

cls_prediction = cls_model.transform(test)
# cal_run_time(start_time, time.time())

print('Prediction result: ')
cls_prediction.select('features', 'readmitted', 'prediction').show(5)
...
cls_model.write().overwrite().save("hdfs://172.31.58.70:9000/data/model")
